...
- latest old-card update: `temperance` / «Умеренность» received approved `preview/full/share` texts and a new 1024x1536 visual with one angel pouring water between two handleless cups, one foot in water and one on land, irises, a path to the horizon and the standard integrated `XIV` marker
...
- cards-first continues in parallel one card per pass; latest pass fully updated `temperance` texts and visual; the next pass can be one new card or one old-card update, without changing IDs, draw/save/journal mechanics, or starting `three-of-swords`
