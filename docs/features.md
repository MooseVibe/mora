...
Cards-first продолжается параллельно маленькими проходами: новая карта через visual/marker/text QA или старая карта через `preview/full/share`. Последний cards-first проход — `five-of-swords` / «Пятёрка Мечей» добавлена как новая карта; следующий проход можно снова брать одной отдельной картой без смешивания с MLP отклика.
