...
- «Поделиться» открывает системный share sheet на mobile/touch с собранным JPEG-шаблоном: фон Mora, арт в рамке, динамический тег и название карты. Если template/file-share недоступен, используется исходный WebP карты, затем text-only fallback. Desktop открывает браузерную отправку Telegram без изображения.
