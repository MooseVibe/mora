...
- Подготовлено к релизу 2026-09-03: DPR cap 3D-карт поднят до 2; loading captions меняются на 8/20 секунде, троеточие примыкает к текущей фразе, тестовый минимум ожидания 10 секунд удалён. Backend-генерация не менялась; фактическую задержку AI ещё нужно замерить.
