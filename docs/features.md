...
### Переходы расклада — релиз 2026-09-05

- Desktop: горизонтальные секции; flip между «Прошлым», «Настоящим» и «Будущим» связан с прогрессом свайпа.
- «Общий взгляд ↔ Прошлое»: реальный DOM-компонент карты перемещается с синхронным изменением изображения, рамки и отступов; handoff без предварительного fade всей секции.
- Mobile flow сохранён. После deploy остаётся ручная проверка плавности и появления текста в обоих направлениях.
...
- Выпущено 2026-09-03 (`38abf16`): DPR cap 3D-карт поднят до 2; loading captions меняются на 8/20 секунде, троеточие примыкает к текущей фразе, тестовый минимум ожидания 10 секунд удалён. Backend-генерация не менялась; фактическую задержку AI ещё нужно замерить.
