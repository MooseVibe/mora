...
Email-вход Mora использует Supabase Auth OTP. Основной сценарий для пользователя: ввести email, получить 6-значный код, ввести код, попасть в dashboard.
...
<p>Введи этот 6-значный код на странице входа:</p>
...
## Что нельзя настроить в текущем Supabase OTP

`{{ .Token }}` — это 6-значный код. Длину кода через Supabase Dashboard не настраиваем. Если понадобится 4/8 цифр, другой TTL или полный контроль над попытками, нужен отдельный custom OTP backend.
