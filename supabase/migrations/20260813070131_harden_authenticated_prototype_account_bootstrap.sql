...
create or replace function mora_private.bootstrap_own_prototype_account(
  p_card_id text,
...
revoke all on function mora_private.bootstrap_own_prototype_account(text, smallint)
...
set search_path = pg_catalog, public, pg_temp
as $$
  select mora_private.bootstrap_own_prototype_account(p_card_id, p_variant_index);
$$;
...
grant execute on function public.bootstrap_own_prototype_account(text, smallint)
  to authenticated;
grant usage on schema mora_private to authenticated;
grant execute on function mora_private.bootstrap_own_prototype_account(text, smallint)
  to authenticated;
