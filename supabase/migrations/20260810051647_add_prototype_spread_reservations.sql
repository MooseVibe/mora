alter table public.prototype_testers
  add column if not exists spread_reservation_id uuid,
  add column if not exists spread_reserved_at timestamptz;

alter table public.prototype_testers
  add constraint prototype_testers_spread_reservation_complete
  check (
...
begin
  ...
  if tester.last_spread_at is not null
    and tester.last_spread_at > reserved_at - interval '24 hours'
  then
...
create or replace function public.release_prototype_spread(
  p_token_hash text,
  p_reservation_id uuid
)
returns jsonb
...
begin
  ...
  return jsonb_build_object('released', released_count = 1);
end;
$$;
...
revoke execute on function public.release_prototype_spread(text, uuid) from public, anon, authenticated;
...
grant execute on function public.release_prototype_spread(text, uuid) to service_role;
